...
def draw_champ_world_icon(game, champ, pos, size, draw_distance = False, draw_hp_bar = False, draw_invisible_duration = False):
	...
	pos.x -= size/2.0
	pos.y -= size/2.0
	game.draw_image(champ.name.lower() + "_square", pos, pos.add(Vec2(size, size)), ColorWhite if champ.is_visible else ColorGray, 100.0)
	
	# Draw hp bar
	if draw_hp_bar:
		pos.y += size
		game.draw_rect_filled(Vec4(pos.x, pos.y, pos.x + size, pos.y + size_hp_bar), Color.BLACK)
		game.draw_rect_filled(Vec4(pos.x + 1, pos.y + 1, pos.x + 1 + (size - 1)*percent_hp, pos.y + size_hp_bar - 1), ColorGreen)
...
def show_on_line(game, champ):
	if not champ.is_alive or not champ.is_visible or champ.is_ally_to(game.player):
		return
	...
	pos = game.world_to_screen(champ.pos.sub(game.player.pos).normalize().scale(game.player.atk_range).add(game.player.pos))
	game.draw_line(game.world_to_screen(game.player.pos), pos, 3, Color.RED)	
	draw_champ_world_icon(game, champ, pos, 48.0, True, True, True)

def show_last_pos_world(game, champ):
	if champ.is_visible or not champ.is_alive or not game.is_point_on_screen(champ.pos):
		return
	draw_champ_world_icon(game, champ, game.world_to_screen(champ.pos), 50.0, False, True, True)
	
def show_last_pos_minimap(game, champ):
	if champ.is_visible or not champ.is_alive:
		return
		
	draw_champ_world_icon(game, champ, game.world_to_minimap(champ.pos), 32.0, False, True, True)
	
def blitz_update(game, ui):
	global bound_max, show_alert_enemy_close, show_last_enemy_pos, show_last_enemy_pos_minimap
	for champ in game.champs:
		if champ.movement_speed > 0:
