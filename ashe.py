...
def blitz_draw_settings(game, ui):
    ...
    if ui.treenode("Setting [E]"):
        use_w_in_combo = ui.checkbox("Use E in Combo", use_w_in_combo)
...
def Combo(game):
    ...
    if use_q_in_combo and IsReady(game, q_spell):
        ...
        if ValidTarget(target) and target and q_spell.name != "YasuoQ3Wrapper":
    ...
    if use_q_in_combo and IsReady(game, q_spell):
        target = targeting.get_target(game, q3['Range'])
        if ValidTarget(target) and target and q_spell.name == "YasuoQ3Wrapper":
            if game.player.pos.distance(target.pos) <= q3['Range']:
                game.move_cursor(game.world_to_screen(target.pos))
                game.press_right_click()
                q_spell.trigger(False)
                # time.sleep(0.02)
                # game.move_cursor(before_cpos)
    if use_w_in_combo and IsReady(game, e_spell):
    ...
    if use_r_in_combo and IsReady(game, r_spell):
        ...
        if ValidTarget(target) and target:
            if target.pos.distance(game.player.pos) <= r['Range']:
                if ultIsKillable(game, target) and target.health >= RDamage(game, target):
                    r_spell.trigger(False)

def LaneClear(game):
    q_spell = getSkill(game, 'Q')
    before_cpos = game.get_cursor()
    if q_spell and q_spell.name != "YasuoQ3Wrapper":
        ...
        if enemyMinion and IsReady(game, q_spell):
            ...
            if (QDamage(game, enemyMinion) >= enemyMinion.health + delay / 2):
                ...
                game.press_right_click()
                q_spell.trigger(False)
    if q_spell and q_spell.name == "YasuoQ3Wrapper" and use_q_tornado_in_laneclear:
        enemyMinion = find_minion_target(game, q3["Range"])
        if enemyMinion and IsReady(game, q_spell):
            game.move_cursor(game.world_to_screen(enemyMinion.pos))
            game.press_right_click()
            q_spell.trigger(False)
...
def Harass(game):
    ...
    r_spell = getSkill(game, 'R')
    before_cpos = game.get_cursor()
    if ValidTarget(target) and target and target.is_enemy_to(game.player) and game.is_point_on_screen(target.pos) and q_spell.name != "YasuoQ3Wrapper":
        if game.player.pos.distance(target.pos) <= q['Range']:
            game.move_cursor(game.world_to_screen(target.pos))
            game.press_right_click()
            q_spell.trigger(False)
...
def blitz_update(game, ui):
    ...
    if draw_q_range:
        ...
        game.draw_circle_world(game.player.pos, q3["Range"], 100, 5, Color.RED)
    if draw_e_range:
