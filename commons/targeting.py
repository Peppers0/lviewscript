...
class TargetingConfig:
	...
	def find_target(self, game, array, range, value_extractor):
		...
		for obj in array:
			...
			if not obj.is_alive or not obj.isTargetable or not obj.is_visible or obj.is_ally_to(game.player) or game.distance(game.player, obj) >= range:
				continue
			if obj.name in self.clones and obj.R.name == obj.D.name:
				continue
